...
nixie-homes holds [home-manager](https://github.com/nix-community/home-manager)
...
- **Integrated**, consumed by nixie's own NixOS/darwin-managed home-manager via the
  `nixie-homes` flake input's `homeModules.<name>` outputs, the same way nixie already
  consumes `nix-secrets` and `keytabs-matos-cc` for secrets — except this is a real
...
commit and push. If nixie needs it, run `nix flake lock --update-input nixie-homes`
